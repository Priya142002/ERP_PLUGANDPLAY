...
import { Plus, CreditCard, Download, Edit, Trash2, Calendar, AlertTriangle } from "lucide-react";
...
export const CustomerPaymentsPage: React.FC = () => {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; payment: any | null }>({ 
    isOpen: false, 
    payment: null 
  });
  ...
  const handleDeleteClick = (item: any) => {
    setDeleteModal({ isOpen: true, payment: item });
  };

  const handleDeleteConfirm = () => {
    if (deleteModal.payment) {
      setPayments(payments.filter(p => p.id !== deleteModal.payment.id));
      setDeleteModal({ isOpen: false, payment: null });
    }
  };

  const handleExport = () => {
    const headers = ['Payment #', 'Date', 'Customer', 'Amount Paid', 'Method', 'Reference'];
    ...
    exportSingleSheetToExcel(headers, data, 'Customer_Payments');
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Customer Payment</h1>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <p className="text-base font-medium text-slate-900 mb-2">
                Are you sure you want to delete "{deleteModal.payment?.paymentNo}"?
              </p>
