...
namespace ERPPlugandPlay.Controllers
{
    ...
    [Route("api/accounts")]
    [Authorize]
    public class AccountsControllerEnhanced : ControllerBase
    {
        private readonly ERPDbContext _context;
        private readonly IAccountingService _accountingService;

        public AccountsControllerEnhanced(ERPDbContext context, IAccountingService accountingService)
        {
            _context = context;
        ...
        public async Task<IActionResult> CreateJournalVoucher([FromBody] CreateJournalVoucherDto dto)
        {
            try
            {
                var voucher = await _accountingService.CreateJournalEntryAsync(dto, GetUserId());
        ...
        public async Task<IActionResult> PostJournalVoucher(int id)
        {
            try
            {
                var voucher = await _accountingService.PostJournalVoucherAsync(id, GetUserId());
        ...
        public async Task<IActionResult> GetAccountLedger(int accountId, [FromQuery] DateTime fromDate, [FromQuery] DateTime toDate)
        {
            try
            {
                var ledger = await _accountingService.GetAccountLedgerAsync(accountId, fromDate, toDate);
        ...
        public async Task<IActionResult> GetTrialBalance(int companyId, int financialYearId, [FromQuery] DateTime asOnDate)
        {
            try
            {
                var trialBalance = await _accountingService.GetTrialBalanceAsync(companyId, financialYearId, asOnDate);
        ...
        public async Task<IActionResult> GetAccountBalance(int accountId, [FromQuery] DateTime? asOnDate = null)
        {
            try
            {
                var balance = await _accountingService.GetAccountBalanceAsync(accountId, asOnDate);
